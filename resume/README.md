...
세 문서 모두 동기화 완료. 1page는 분량 때문에 팀 구성 인원을 생략합니다.

> ⚠️ `CAREER.md` §7에 **문서 간 수치 불일치 8건**이 미해결로 남아 있습니다.
> (쿠차 배치 신규 건수 12 vs 32 등) 확정 값을 아시면 §7을 정리하고 세 문서에 반영하세요.

### index.html 관련 주의

- `data-lucide="github"` 는 **렌더되지 않습니다.** lucide가 브랜드 아이콘을 제거해서
  `<i>` 태그가 SVG로 치환되지 않고 빈 칸으로 남습니다. `git-branch` 로 대체해 뒀습니다.
  아이콘을 추가할 때는 실제로 SVG가 생기는지 확인하세요:

  ```js
  document.querySelectorAll('.contact-link').forEach(a =>
    console.log(a.textContent.trim(), a.querySelector('svg') ? 'OK' : 'NO ICON'))
  ```

- 웹 이력서는 Tailwind CDN과 Google Fonts를 사용합니다. PDF 인쇄본과 달리
  네트워크에 의존하므로, 두 문서의 폰트 렌더링은 의도적으로 다릅니다.
